...
from datasets.stats import process_split
from models.common import knn, rigid_transform_3d
from utils.SE3 import transform
from utils.pointcloud import refine
from utils.build_graphs import build_graphs
from models.dgcnn import DGCNN
from models.gconv import Siamese_Gconv
from models.transformer import Transformer
import math
from models.computeLRF import compute_LRF_batch, compute_neighborhood, compute_LRF_affinity, compute_angles_between_normals
...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, feat, attention):
        ...
        weight = torch.softmax(attention[:, None, :, :] * feat_attention, dim=-1) #b 1 n n
        message = torch.einsum('bhoi, bhci-> bhco', weight, V).reshape([bs, -1, num_corr])  #b c n
...
class PointDSC(nn.Module):
    def __init__(self,
        ...
        self.pointfeaturer = DGCNN(['xyz','gxyz'], 20, 128)
        self.gnn_layer = 5
        for i in range(self.gnn_layer):
            if i == 0:
                gnn_layer = Siamese_Gconv(128 + 128, 128)
            else:
                gnn_layer = Siamese_Gconv(128, 128)
            self.add_module('gnn_layer_{}'.format(i), gnn_layer)
            # self.add_module('gmattend{}'.format(i), Transformer(2 * 128
            #                                                         if i == 0 else 128))
            self.add_module('gmattend{}'.format(i), NonLocalBlock(2 * 128
                                                                if i == 0 else 128))
        ...
        self.sigma_lrf = nn.Parameter(torch.Tensor([5.0]).float(), requires_grad=False)
    ...
    def forward(self, data):
        ...
        """
        corr_pos, src_keypts, tgt_keypts, src_normals, tgt_normals = data['corr_pos'], data['src_keypts'], data['tgt_keypts'], data['src_normals'], data['tgt_normals']
        ...
        with torch.no_grad():
            ...
            dist_compatibility = src_dist - torch.norm((tgt_keypts[:, :, None, :] - tgt_keypts[:, None, :, :]), dim=-1)
            # corr_compatibility = torch.clamp(1.0 - dist_compatibility ** 2 / self.sigma_spat ** 2, min=0)
            #计算法向量之间的夹角
            # src_angle = compute_angles_between_normals(src_normals)
            # tgt_angle = compute_angles_between_normals(tgt_normals)
            # angle_compatibility = torch.abs(src_angle - tgt_angle)
            # angle_compatibility_scroes = torch.clamp(1.0 - angle_compatibility / self.sigma_lrf, min=0)
            # # Consistency_scores = weight1 * corr_compatibility + weight2 * angle_compatibility_scroes
            # Consistency_scores = torch.exp(- weight1 * (dist_compatibility ** 2 / self.sigma_spat ** 2) - weight2 * angle_compatibility_scroes)

            #计算LRF
            neighbors_src = compute_neighborhood(src_keypts, k=k_LRF_neigh)
            neighbors_tgt = compute_neighborhood(tgt_keypts, k=k_LRF_neigh)
            LRF_src, LRF_tgt = compute_LRF_batch(src_keypts, tgt_keypts, neighbors_src, neighbors_tgt, src_normals, tgt_normals)
            LRF_compatibility = compute_LRF_affinity(LRF_src, LRF_tgt)
            LRF_compatibility = LRF_compatibility ** 2 / self.sigma_lrf ** 2
            LRF_compatibility = torch.clamp(1.0 - LRF_compatibility / self.sigma_lrf, min=0)
            # Consistency_scores = weight1*corr_compatibility + weight2*LRF_compatibility
            Consistency_scores = torch.exp(-(dist_compatibility ** 2 / self.sigma_spat ** 2) - LRF_compatibility)
            # Consistency_scores = torch.exp(-(dist_compatibility ** 2 / self.sigma_spat ** 2))


        USEATTEND = 'attentiontransformer'
        # A1_gt, e1_gt = build_graphs(corr_pos, corr_pos, num_corr)
        Node_corr, Edge_corr = self.pointfeaturer(corr_pos)
        emb_corr = torch.cat((Node_corr, Edge_corr), dim=1).transpose(1, 2).contiguous()   #emb_corr b n c

        for i in range(self.gnn_layer):
            gnn_layer = getattr(self, 'gnn_layer_{}'.format(i))
            if USEATTEND == 'attentiontransformer':
                gmattends_layer = getattr(self, 'gmattend{}'.format(i))

                corr_embedding = gmattends_layer(emb_corr.permute(0,2,1),Consistency_scores)   #corr_embedding b c n
                # corr_embedding = gmattends_layer(emb_corr, Consistency_scores)
                d_k = corr_embedding.size(1)
                scores_src = torch.matmul(corr_embedding.transpose(2, 1).contiguous(), corr_embedding) / math.sqrt(d_k)
                A_corr1 = torch.softmax(scores_src, dim=-1)
                # emb_corr = gnn_layer([A_corr1, emb_corr])   #n n 和n c
                emb_corr = gnn_layer([A_corr1, corr_embedding.transpose(2, 1)])
        ...
        # emb_corr = self.Ablation(corr_pos)
        # corr_features = self.encoder(corr_pos.permute(0,2,1), corr_compatibility).permute(0, 2, 1)
        normed_corr_features = F.normalize(emb_corr, p=2, dim=-1)
        ...
        confidence = self.cal_leading_eigenvector(Consistency_scores, method='power')
        # confidence = self.classification(emb_corr.permute(0, 2, 1)).squeeze(1)
        
        if testing:
            seeds = self.pick_seeds(src_dist, confidence, R=self.nms_radius, max_num=int(num_corr * self.ratio))
            # seeds = self.grid_partition_and_select_topk(src_keypts, confidence, grid_size=0.5, k=int(num_corr * self.ratio))
    ...
    def grid_partition_and_select_topk(self,points_batch, confidence_batch, grid_size=0.5, k=None):
        """
        对批次点云进行网格划分，并在每个网格中循环取点，直到总共取出 k 个点，
        然后从这些点中选出置信度最高的 k2 个点

        参数：
        points_batch (Tensor): 维度为 (b, n, 3) 的批次点云坐标
        confidence_batch (Tensor): 维度为 (b, n) 的批次置信度
        grid_size (float): 网格的边长
        k (int): 点的总数量

        返回：
        Tensor: 选出的点的索引，维度为 (b, k)
        """
        points = points_batch[0]
        confidence = confidence_batch[0]
        ...
        # 计算每个点所属的网格索引
        grid_indices = ((points - min_coords) / grid_size).floor().int()

        # 获取每个 grid_index 的唯一值和其在 grid_indices 中的对应索引
        unique_grid_indices, inverse_indices = torch.unique(grid_indices, dim=0, return_inverse=True)
        # 创建一个空的字典来存储结果
        grid_to_points = {tuple(unique_grid_indices[i].tolist()): [] for i in range(unique_grid_indices.shape[0])}
        # 遍历 inverse_indices 并将每个索引添加到对应的 grid_index 的列表中
        for idx, inverse_idx in enumerate(inverse_indices):
            key = tuple(unique_grid_indices[inverse_idx].tolist())
            grid_to_points[key].append(idx)
        # 循环从每个网格中取一个点，直到取出 k 个点
        grid_to_points = {torch.tensor(k, device=points.device): torch.tensor(v, device=points.device) for k, v in grid_to_points.items()}

        selected_indices = []
        selected_set = set()

        for grid_index, point_indices in grid_to_points.items():
            # 将 point_indices 和 selected_set 转换为 GPU 上的 tensor
            point_indices_tensor = torch.tensor(point_indices, device=points.device)

            # 过滤出还没有被选中的点
            mask = ~torch.isin(point_indices_tensor, torch.tensor(list(selected_set), device=points.device))
            remaining_points = point_indices_tensor[mask]

            if len(remaining_points) > 0:
                # 找到置信度最高的点
                highest_confidence_idx = remaining_points[torch.argmax(confidence[remaining_points])]
                selected_indices.append(highest_confidence_idx.item())
                selected_set.add(highest_confidence_idx.item())

        selected_indices = torch.tensor(selected_indices[:]).to(points.device)

        num_needed = k - len(selected_indices)

        remaining_indices = torch.arange(len(confidence), device=points.device)
        remaining_indices = remaining_indices[
            ~torch.isin(remaining_indices, torch.tensor(list(selected_indices), device=points.device))]

        # 按置信度对剩余点进行排序（降序）
        _, sorted_indices = torch.sort(confidence[remaining_indices], descending=True)

        # 从排序后的剩余点中选择所需的点
        final_selected_points = torch.cat((selected_indices, remaining_indices[sorted_indices][:num_needed]), dim=0)

        return final_selected_points.unsqueeze(0)
    ...
    def cal_seed_trans(self, seeds, corr_features, src_keypts, tgt_keypts):
        ...
        # local_feature_M = ((1-knn_M) < self.sigma).float()
        # local_SC2_measure = torch.matmul(local_feature_M[:, :, :1, :], local_feature_M)
        # sorted_score = torch.argsort(local_SC2_measure, dim=3, descending=True)
        # knn_idx_fine = sorted_score[:, :, :, 0: k2]
        # knn_idx_fine = knn_idx_fine.contiguous().view([bs, num_seeds, -1])[:, :, :, None]
        # knn_idx_fine = knn_idx_fine.expand(-1, -1, -1, num_channels)
        # feature_knn_fine = knn_features.gather(dim=2, index=knn_idx_fine).view([bs, -1, k2, num_channels])
        # knn_M_fine = torch.matmul(feature_knn_fine, feature_knn_fine.permute(0, 1, 3, 2))
        # knn_M_feature = torch.clamp(1 - (1 - knn_M_fine) / self.sigma ** 2, min=0)
        # knn_M_feature = knn_M_feature.view([-1, k2, k2])
        # feature_knn_M = knn_M_feature

        #################################
        # construct the spatial consistency matrix of each correspondence subset.
        #################################
        #先在特征空间下挑k1个，然后再在长度一致性下挑k2个
        ...
        # #消融
        # total_knn_knn = torch.matmul(knn_features, knn_features.permute(0, 1, 3, 2)) * knn_M
        # total_knn_knn = total_knn_knn.view([-1, k, k])
        # #消融

        #先采40个最好的邻居，计算一次长度一致性，然后得到矩阵，然后在矩阵里找到最好的20个
        local_hard_SC_measure = (knn_M < self.sigma_spat).float()
        local_SC2_measure = torch.matmul(local_hard_SC_measure[:, :, :1, :], local_hard_SC_measure)

        sorted_score = torch.argsort(local_SC2_measure, dim=3, descending=True)
        knn_idx_fine = sorted_score[:, :, :, 0: k2]
        knn_idx_fine = knn_idx_fine.contiguous().view([bs, num_seeds, -1])[:, :, :, None]
        knn_idx_fine_s = knn_idx_fine.expand(-1, -1, -1, 3)
        knn_idx_fine_f = knn_idx_fine.expand(-1, -1, -1, num_channels)
        # 拿到k2个点对的索引，取得点对和特征
        src_knn_fine = src_knn.gather(dim=2, index=knn_idx_fine_s).view([bs, -1, k2, 3])  # [bs, num_seeds, k, 3]
        tgt_knn_fine = tgt_knn.gather(dim=2, index=knn_idx_fine_s).view([bs, -1, k2, 3])
        #对选择出来的计算特征一致性
        feature_knn_fine = knn_features.gather(dim=2, index=knn_idx_fine_f).view([bs, -1, k2, num_channels])
        knn_M_fine = torch.matmul(feature_knn_fine, feature_knn_fine.permute(0, 1, 3, 2))
        knn_M_feature = torch.clamp(1 - (1 - knn_M_fine) / self.sigma ** 2, min=0)
        knn_M_feature = knn_M_feature.view([-1, k2, k2])
        feature_knn_M = knn_M_feature
        ...
        local_SC_measure = torch.clamp(1 - cross_dist ** 2 / self.sigma_spat ** 2, min=0)
        local_hard_measure = (cross_dist < self.sigma_spat * 2).float()
        ...
        # total_weight = total_weight.view([bs, -1, k])  #消融
    ...
    def post_refinement(self, initial_trans, src_keypts, tgt_keypts, weights=None):
        ...
        # initial_trans = refine(src_keypts, tgt_keypts, initial_trans)
