...
def process_3dmatch(voxel_size=0.05):
    root = "../dataset/threedmatch"
    save_path = "../dataset/threedmatch_feat"
    ...
    for pcd_path in pcd_list:
        ...
        pcd_normals.points = o3d.utility.Vector3dVector(xyz_down)
        pcd_normals.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
...
def process_3dmatch_test(voxel_size=0.05):
    ...
    for scene in scene_list:
        scene_path = os.path.join("../dataset/threedmatch_test/fragments/", scene)
...
def process_kitti(voxel_size=0.30, split='train'):
    ...
    for idx in range(len(files)):
        ...
        pcd_normals0 = o3d.geometry.PointCloud()
        pcd_normals0.points = o3d.utility.Vector3dVector(xyz_down0)
        pcd_normals0.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.6, max_nn=30))
        normals0 = np.asarray(pcd_normals0.normals)
        pcd_normals1 = o3d.geometry.PointCloud()
        pcd_normals1.points = o3d.utility.Vector3dVector(xyz_down1)
        pcd_normals1.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.6, max_nn=30))
        normals1 = np.asarray(pcd_normals1.normals)
        ...
        np.savez_compressed(
            ...
            normals0=normals0.astype(np.float32),
            normals1=normals1.astype(np.float32),
            gt_trans=M2
        )
        print(filename)
