...
def process_kitti(voxel_size=0.30, split='train'):
    ...
    for idx in range(len(files)):
        ...
        fpfh0 = o3d.pipelines.registration.compute_fpfh_feature(pcd0, o3d.geometry.KDTreeSearchParamHybrid(radius=voxel_size * 5, max_nn=100))
        ...
        fpfh1 = o3d.pipelines.registration.compute_fpfh_feature(pcd1, o3d.geometry.KDTreeSearchParamHybrid(radius=voxel_size * 5, max_nn=100))
        ...
        np.savez_compressed(
            ...
            normals0=np.array(pcd0.normals).astype(np.float32),
            normals1=np.array(pcd1.normals).astype(np.float32),
            gt_trans=M2
        )
        print(filename)
