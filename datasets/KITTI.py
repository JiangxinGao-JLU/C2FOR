...
class KITTIDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        src_pcd, tgt_pcd = self.get_pcd(filename)
        ...
        src_normals = data['normals0']
        tgt_normals = data['normals1']
        ...
        src_normals = src_normals[src_sel_ind, :]
        tgt_normals = tgt_normals[tgt_sel_ind, :]
        ...
        labels = (distance < self.inlier_threshold).astype(np.int64)
        ...
        if self.split == 'train' and np.mean(labels) > 0.5:
            ...
            src_normals_outliers = np.random.randn(num_outliers, 3) * np.mean(src_normals, axis=0)
            tgt_normals_outliers = np.random.randn(num_outliers, 3) * np.mean(tgt_normals, axis=0)
            ...
            input_src_normals = np.concatenate([src_normals[corr[:, 0]], src_normals_outliers], axis=0)
            input_tgt_normals = np.concatenate([tgt_normals[corr[:, 1]], tgt_normals_outliers], axis=0)
            ...
            input_src_normals = src_normals[corr[:, 0]]
            input_tgt_normals = tgt_normals[corr[:, 1]]
        ...
        if self.split=='test':
            return src_pcd.astype(np.float32), tgt_pcd.astype(np.float32), corr_pos.astype(np.float32), \
                input_src_keypts.astype(np.float32), \
                input_tgt_keypts.astype(np.float32), \
                input_src_normals.astype(np.float32), \
                input_tgt_normals.astype(np.float32), \
                gt_trans.astype(np.float32), \
                labels.astype(np.float32),
        else:
            return np.identity(4), np.identity(4), corr_pos.astype(np.float32), \
                input_src_keypts.astype(np.float32), \
                input_tgt_keypts.astype(np.float32), \
                input_src_normals.astype(np.float32), \
                input_tgt_normals.astype(np.float32), \
    ...
    def get_pcd(self,filename):
        file_name_no_ext = os.path.splitext(filename)[0]

        # 按 '-' 和 '_' 分割
        parts = file_name_no_ext.split('-')
        drive_str = parts[0]  # 'drive6'
        pair = parts[1].split('_')  # ['pair45', '53']

        # 提取数字部分
        drive_number = int(drive_str[-1])
        pair_first_number = int(pair[0].replace('pair', ''))  # 45
        pair_second_number = int(pair[1])  # 53
        drive_number = str(drive_number).zfill(2)
        pair_first_number_str = str(pair_first_number).zfill(6)
        pair_second_number_str = str(pair_second_number).zfill(6)

        fsrcnames = f"{self.root}/sequences/{drive_number}/velodyne/{pair_first_number_str}.bin"
        ftgtnames = f"{self.root}/sequences/{drive_number}/velodyne/{pair_second_number_str}.bin"
        xyzr0 = np.fromfile(fsrcnames, dtype=np.float32).reshape(-1, 4)
        xyzr1 = np.fromfile(ftgtnames, dtype=np.float32).reshape(-1, 4)
        src_pcd = xyzr0[:, :3]
        tgt_pcd = xyzr1[:, :3]
