...
def get_predator_data(pair_idx, n_points):
    data_dict = torch.load(f'./dataset/3DLoMatch-data/{pair_idx}.pth')
    len_src = data_dict['len_src']
    src_pcd = data_dict['pcd'][:len_src, :].cuda(gpu_num)
    tgt_pcd = data_dict['pcd'][len_src:, :].cuda(gpu_num)
    src_feats = data_dict['feats'][:len_src].cuda(gpu_num)
    tgt_feats = data_dict['feats'][len_src:].cuda(gpu_num)
    ...
    srcpcd.points = o3d.utility.Vector3dVector(src_pcd.cpu().numpy())
    tgtpcd = o3d.geometry.PointCloud()
    tgtpcd.points = o3d.utility.Vector3dVector(tgt_pcd.cpu().numpy())
    srcpcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
    tgtpcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
    src_normals = np.asarray(srcpcd.normals).astype(np.float32)
    src_normals = torch.from_numpy(src_normals).cuda(gpu_num)
    tgt_normals = np.asarray(tgtpcd.normals).astype(np.float32)
    tgt_normals = torch.from_numpy(tgt_normals).cuda(gpu_num)
    ...
    data = {
        ...
        'src_normals': src_normals[None],
        'tgt_normals': tgt_normals[source_idx][None],
        'testing': True,
    }
    
    gt_trans = integrate_trans(data_dict['rot'], data_dict['trans']).cuda(gpu_num)
    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            if args.descriptor == 'fcgf':
                ...
                src_pcd, tgt_pcd, corr, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = next(dloader_iter)
                corr, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = \
                    corr.cuda(gpu_num), src_keypts.cuda(gpu_num), tgt_keypts.cuda(gpu_num), src_normals.cuda(gpu_num), tgt_normals.cuda(gpu_num), gt_trans.cuda(gpu_num), gt_labels.cuda(gpu_num)
                data = {
                    'corr_pos': corr,
                ...
                src_normals, tgt_normals = data['src_normals'], data['tgt_normals']
            ...
            # if i == 10:
            #     torch.save(pred_trans[0], 'pred_trans_3DLoMatch.pth')
...
def eval_3DMatch(model, config, args):
    dset = ThreeDLOMatchTest(root='./dataset/threedmatch_test',
                            descriptor='fcgf',
    ...
    benchmark_predator(allpair_poses, gt_folder='./dataset/3DLoMatch')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='./snapshot/PointDSC_3DMatch_10101724', type=str, help='snapshot dir')
    parser.add_argument('--descriptor', default='predator', type=str, choices=['fcgf', 'predator'])
    ...
    config_path = f'{args.chosen_snapshot}/config.json'
    config = json.load(open(config_path, 'r'))
    config = edict(config)

    p = Path(args.chosen_snapshot)
    snap_name = p.stem if p.suffix else p.name

    log_dir = Path("./evaluation/logs")
    log_dir.mkdir(parents=True, exist_ok=True)

    suffix = "-ICP" if getattr(args, "use_icp", False) else ""
    log_file = log_dir / f"{snap_name}-{args.descriptor}-{args.num_points}{suffix}.log"
    ...
    miss = model.load_state_dict(torch.load(f'{args.chosen_snapshot}/models/model_best.pkl'), strict=False)
    print(miss)
    model.eval()

    # evaluate on the test set
    stats = eval_3DMatch(model.cuda(gpu_num), config, args)
