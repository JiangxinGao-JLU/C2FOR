    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            corr, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = \
                corr.cuda(gpu_mun), src_keypts.cuda(gpu_mun), tgt_keypts.cuda(gpu_mun), src_normals.cuda(gpu_mun), tgt_normals.cuda(gpu_mun), gt_trans.cuda(gpu_mun), gt_labels.cuda(gpu_mun)
            data = {
                'corr_pos': corr,
            ...
            # if i ==10:
            #     torch.save(pred_trans[0], 'pred_trans_3DMatch.pth')
            #     torch.save(src_pcd[0], 'src_pcd.pth')
            #     torch.save(tgt_pcd[0], 'tgt_pcd.pth')
            #     torch.save(gt_trans[0],'gt_trans.pth')
...
def eval_3DMatch(model, config, use_icp):
    ...
    for scene_ind, scene in enumerate(scene_list):
        dset = ThreeDMatchTest(root='./dataset/threedmatch_test',
...
if __name__ == '__main__':
    ...
    log_file = log_dir / f"{snap_name}-{args.solver}{suffix}-{config.descriptor}.log"
    ...
    stats = eval_3DMatch(model.cuda(gpu_mun), config, args.use_icp)
