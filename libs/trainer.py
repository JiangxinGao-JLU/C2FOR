...
class Trainer(object):
    def __init__(self, args):
        ...
        self.inlier_threshold = args.inlier_threshold
        ...
        if self.gpu_mode:
            self.model = self.model.cuda(self.gpu_num)
    ...
    def train_epoch(self, epoch):
        ...
        for iter in range(num_iter):
            data_timer.tic()
            (src_pcd, tgt_pcd, corr_pos, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels) = next(trainer_loader_iter)
            if self.gpu_mode:
                corr_pos, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = \
                    corr_pos.cuda(self.gpu_num), src_keypts.cuda(self.gpu_num), tgt_keypts.cuda(self.gpu_num), src_normals.cuda(self.gpu_num), tgt_normals.cuda(self.gpu_num), gt_trans.cuda(self.gpu_num), gt_labels.cuda(self.gpu_num)
            data = {
                'corr_pos': corr_pos,
    ...
    def evaluate(self, epoch):
        ...
        for iter in range(num_iter):
            data_timer.tic()
            (src, tgt, corr_pos, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels) = next(val_loader_iter)
            if self.gpu_mode:
                corr_pos, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = \
                    corr_pos.cuda(self.gpu_num), src_keypts.cuda(self.gpu_num), tgt_keypts.cuda(
                        self.gpu_num), src_normals.cuda(self.gpu_num), tgt_normals.cuda(self.gpu_num), gt_trans.cuda(
                        self.gpu_num), gt_labels.cuda(self.gpu_num)
            data = {
                'corr_pos': corr_pos,
                'src_keypts': src_keypts,
                'tgt_keypts': tgt_keypts,
