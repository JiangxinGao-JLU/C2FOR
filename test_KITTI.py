...
def eval_KITTI_per_pair(model, dloader, config, use_icp):
    ...
    with torch.no_grad():
        for i in tqdm(range(num_pair)):
            ...
            corr, src_keypts, tgt_keypts, src_normals, tgt_normals, gt_trans, gt_labels = corr.cuda(gpu_num), src_keypts.cuda(gpu_num), tgt_keypts.cuda(gpu_num), src_normals.cuda(gpu_num), tgt_normals.cuda(gpu_num), gt_trans.cuda(gpu_num), gt_labels.cuda(gpu_num)
            data = {
                'corr_pos': corr,
...
def eval_KITTI(model, config, use_icp):
    dset = KITTIDataset(root='./dataset/kitti',
                    split='test',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='./snapshot/PointDSC_KITTI_08012130', type=str, help='snapshot dir')
    ...
    log_file = log_dir / f"{snap_name}-{args.solver}{suffix}.log"
    ...
    stats = eval_KITTI(model.cuda(gpu_num), config, args.use_icp)
