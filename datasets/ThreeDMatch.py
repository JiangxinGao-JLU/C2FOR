...
class ThreeDMatchTrainVal(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.descriptor == 'fcgf':
            ...
            src_normals = src_data['normals']
            tgt_normals = tgt_data['normals']
...
class ThreeDMatchTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load point coordinates and pre-computed per-point local descriptors
        if self.descriptor == 'fcgf':
            src_pcd = np.array(o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{src_id}.ply").points)
            tgt_pcd = np.array(o3d.io.read_point_cloud(f"{self.root}/fragments/{scene}/cloud_bin_{tgt_id}.ply").points)
        ...
        return src_pcd.astype(np.float32),\
            tgt_pcd.astype(np.float32),\
            corr_pos.astype(np.float32), \
            input_src_keypts.astype(np.float32), \
            input_tgt_keypts.astype(np.float32), \
            input_src_normals.astype(np.float32), \
            input_tgt_normals.astype(np.float32), \
